def calorie_intake():
    ...
    if gender in ["M","F"]:
        if gender== "M":
            x_male= 66+6.2*(weight)+12.7*(height)-6.76*(age) #x_male is number of calories a male should consume
            answer = x_male
        if gender== "F":
            x_female= 655.1+4.35*(weight)+4.7*(height)-4.7*(age)
            answer = x_female
    elif gender not in ["M","F"]:
        gender = input("Error, please type in gender again, using the single letter M or F")
        if gender== "M":
            x_male= 66+6.2*(weight)+12.7*(height)-6.76*(age) #x_male is number of calories a male should consume
            answer = x_male
        if gender== "F":
